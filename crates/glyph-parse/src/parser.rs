...
impl Parser {
    ...
    pub fn parse_def(&mut self, name: &str, kind: &str) -> Result<Def> {
        ...
        match kind {
            ...
            "trait" => self.parse_trait_def(),
            "impl" => self.parse_impl_def(),
            "const" => self.parse_const_def(),
            "fsm" => self.parse_fsm_def(),
            "srv" => self.parse_srv_def(),
    ...
    fn parse_trait_def(&mut self) -> Result<Def> {
        let start = self.span();
    ...
    fn parse_impl_def(&mut self) -> Result<Def> {
        let start = self.span();
        self.expect(&TokenKind::Impl)?;
        let trait_name = self.expect_type_ident()?;
        ...
        let name = format!("impl_{}_for_{}", trait_name, type_name);
        Ok(Def {
            id: None,
            name,
    ...
    fn parse_const_def(&mut self) -> Result<Def> {
        let start = self.span();
        ...
        Ok(Def {
            id: None,
            name: name.clone(),
            kind: DefKind::Const(ConstDef { ty, value }),
            span: start.merge(self.prev_span()),
        })
    }
    ...
    fn parse_fsm_def(&mut self) -> Result<Def> {
        let start = self.span();
        ...
        while !self.check(&TokenKind::Dedent) && !self.check(&TokenKind::Eof) {
            ...
            let sname = self.expect_type_ident()?;
            self.expect(&TokenKind::Indent)?;
            ...
            while !self.check(&TokenKind::Dedent) && !self.check(&TokenKind::Eof) {
                ...
                self.expect(&TokenKind::Arrow)?;
                let target = self.expect_type_ident()?;
                let action = if self.check(&TokenKind::Slash) {
                    self.advance();
                    Some(self.parse_expr()?)
                } else {
                    None
                ...
                transitions.push(Transition {
                    ...
                    target,
                    action,
                    span: tstart.merge(self.prev_span()),
                });
                self.skip_newlines();
            }
            if self.check(&TokenKind::Dedent) {
            ...
            states.push(StateDef {
                name: sname,
                transitions,
                span: sstart.merge(self.prev_span()),
            });
            self.skip_newlines();
        }
        if self.check(&TokenKind::Dedent) {
        ...
        Ok(Def {
            id: None,
            name: name.clone(),
            kind: DefKind::Fsm(FsmDef { states }),
            span: start.merge(self.prev_span()),
        })
    }
    ...
    fn parse_srv_def(&mut self) -> Result<Def> {
        let start = self.span();
        self.expect(&TokenKind::Srv)?;
        ...
        while !self.check(&TokenKind::Dedent) && !self.check(&TokenKind::Eof) {
            ...
            routes.push(Route {
                path,
                handlers,
                span: rstart.merge(self.prev_span()),
            });
            self.skip_newlines();
        }
        if self.check(&TokenKind::Dedent) {
        ...
        Ok(Def {
            id: None,
            name: "srv".to_string(),
            kind: DefKind::Srv(SrvDef { addr, routes }),
            span: start.merge(self.prev_span()),
        })
    }
    ...
    fn parse_http_method(&mut self) -> Result<HttpMethod> {
        match self.current_kind() {
            ...
            _ => Err(ParseError::UnexpectedToken {
                found: format!("{:?}", self.current_kind()),
                expected: "HTTP method (G, P, U, X, H)".to_string(),
                span: self.span(),
            }),
        }
    }
